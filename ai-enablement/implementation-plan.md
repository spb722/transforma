# transforma × CLI AI — implementation plan
...
**Baseline:** `main` @ `1f40230` (v1.2.3 + dependabot config), fork `spb722/transforma`
...
| ID2 | **Authentication is a UI flow.** The admin connects the instance to the provider from the admin dashboard via the provider's own OAuth flow — no terminal, no `.env`, no file edits. | transforma implements the OAuth (PKCE) dance the CLI itself uses, stores tokens server-side, and injects them into CLI invocations. An API-key field remains as fallback. |
...
| A2 | FR-04: "transforma never stores or asks for model API keys/accounts in its UI; CLI authentication is owned by the instance owner outside transforma." | The **admin** dashboard is now where the instance credential is created (OAuth) or pasted (API key fallback); it is stored server-side, encrypted, never shown back in full. What FR-04 protected is preserved: **profile users** never see, provide, or hold credentials, and the consent screen still names the provider and whose account pays (FR-09). |
...
|---|---|
| `plan-share.js` — `parsePlan` / `mergePlan` | Created plans are `transforma_plan`-shaped bundles; accept = `mergePlan` verbatim (FR-15/16/18). |
...
**C-1. Mechanism.** The Claude Code CLI supports non-interactive auth via an environment variable carrying a long-lived OAuth token (the same token its own `claude setup-token` flow produces). transforma reproduces that flow server-side — **standard OAuth 2.0 authorization-code + PKCE**, using the CLI's public client parameters (extracted from, and CI-verified against, the *pinned* CLI version — see C-4):
...
- **Creation** → an `transforma_plan`-compatible bundle (v1 fields exactly as `plan-share.js` writes them) extended with `summary`, per-routine/per-exercise `why`, and `basedOn` history note (FR-15, B2).
...
10. Licensing posture (NFR-5): the CLI is installed alongside, invoked as a separate process — mere aggregation, AGPL posture unchanged; `NOTICE.md` gains a line; the connect dialog links the provider's terms (the owner accepts them, not transforma).
