/* Coach instance configuration — the one place that knows whether this instance offers the
   ...
   cache, rather than Transforma importing or duplicating an OAuth token. */
...
// This file is deliberately owned by Transforma, not a person's normal Codex configuration. It
...
function key() {
  ...
  keyCache = Buffer.from(crypto.hkdfSync('sha256', Buffer.from(secret, 'utf8'), Buffer.alloc(0), Buffer.from('transforma-coach-v1'), 32));
  return keyCache;
}
...
export function jobEnv(jobDir) {
  ...
  if (cfg.provider === 'codex') {
    ...
    // bind mount owned by `coach`; /data and every Transforma secret remain inaccessible.
