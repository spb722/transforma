/* transforma-api — passkey (WebAuthn) auth + per-user state storage for transforma
...
import { loadCoachingConfig } from './coaching/config.js';
import { openCoachingDb } from './coaching/db.js';
import { coachingRoutes, relationshipHandlers } from './coaching/routes.js';
import { profileService } from './coaching/profiles.js';
import { invitationService } from './coaching/invitations.js';
import { relationshipService } from './coaching/relationships.js';
import { workspaceService } from './coaching/workspaces.js';
import { templateService } from './coaching/templates.js';
import { planService, validateWorkoutContent } from './coaching/plans.js';
import { occurrenceService } from './coaching/occurrences.js';
import { sessionService } from './coaching/sessions.js';
import { validateDietContent } from './coaching/diets.js';
import { checkInService } from './coaching/check-ins.js';
import { progressService } from './coaching/progress.js';
import { historyService } from './coaching/history.js';
...
const RP_NAME = process.env.RP_NAME || 'transforma';
...
function fixedWindowLimiter(max, windowMs) {
...
const allowRegistrationIp = fixedWindowLimiter(10, 10 * 60_000);
const allowRegistrationToken = fixedWindowLimiter(5, 10 * 60_000);
// The direct socket is authoritative. Reverse proxies should enforce their own edge limits;
// arbitrary forwarded headers are deliberately ignored here.
const requestIp = req => req.socket?.remoteAddress || 'unknown';
...
db.coachingAdmissions = db.coachingAdmissions || [];
...
/* ---------- trainer–client coaching ---------- */
const coachingConfig = loadCoachingConfig(process.env);
const coachingStore = openCoachingDb({
  dataDir: DATA,
  identities: {
    findUser: id => db.users.find(user => user.id === id) || null,
    isEnabled: id => {
...
const isTrainer = user => !!user && coachingConfig.trainerIds.has(user.id);
const coachingProfiles = profileService(coachingStore, { config: coachingConfig });
const coachingInvitations = invitationService(coachingStore, { config: coachingConfig });
const coachingOccurrences = occurrenceService(coachingStore);
coachingOccurrences.catchUpAll();
setInterval(() => coachingOccurrences.catchUpAll(), 6 * 60 * 60 * 1000).unref();
const coachingRelationships = relationshipService(coachingStore, { beforeEnd: coachingOccurrences.endRelationship });
const validatePlanContent = (kind, value) => kind === 'workout' ? validateWorkoutContent(value) : validateDietContent(value);
const coachingTemplates = templateService(coachingStore, { validateContent: validatePlanContent });
const coachingPlans = planService(coachingStore, { validateContent: validatePlanContent, beforePublish: (relationshipId, kind) => { if (kind === 'workout') coachingOccurrences.catchUpRelationship(relationshipId) }, afterPublish: coachingOccurrences.reconcile });
const coachingSessions = sessionService(coachingStore, { beforeStart: coachingOccurrences.catchUpAll });
const coachingCheckIns = checkInService(coachingStore);
const coachingProgress = progressService(coachingStore, { readPersonalState: readState });
const coachingHistory = historyService(coachingStore, { readPersonalState: readState });
const coachingFees = feeService(coachingStore, { config: coachingConfig });
const coachingWorkspaces = workspaceService(coachingStore, {
  config: coachingConfig,
  identities: coachingStore.identities,
  readPersonalState: readState,
  occurrences: coachingOccurrences
});
const coachingHandlers = relationshipHandlers({
  store: coachingStore,
  profiles: coachingProfiles,
  invitations: coachingInvitations,
  relationships: coachingRelationships,
  workspaces: coachingWorkspaces,
  templates: coachingTemplates,
  plans: coachingPlans,
  occurrences: coachingOccurrences,
  sessions: coachingSessions,
  checkIns: coachingCheckIns,
  progress: coachingProgress,
  history: coachingHistory,
  fees: coachingFees,
  config: coachingConfig
});
const readCoachingSession = req => {
  const user = readSession(req);
  return user ? { ...user, admin: isAdmin(user) } : null;
};

function projectAssignedResults(userId, state) {
  if (!state) return state;
  const canonical = coachingStore.db.prepare("SELECT id,scheduled_date,prescription_json,actual_json,summary_json,started_at,client_finished_at FROM assigned_sessions WHERE client_id=? AND state='finished'").all(userId).map(row => ({ id:row.id,d:row.scheduled_date,start:Date.parse(row.started_at),end:Date.parse(row.client_finished_at),name:JSON.parse(row.prescription_json).dayName,entries:JSON.parse(row.actual_json||'{\"entries\":[]}').entries||[],assignedSessionId:row.id,assignedPrescription:JSON.parse(row.prescription_json),assignedSync:'synced',summary:JSON.parse(row.summary_json||'null') }));
  const ids = new Set(canonical.map(item => item.id));
  return { ...state, workouts: [...(state.workouts || []).filter(item => !ids.has(item.id)), ...canonical] };
}
...
setInterval(() => {
  for (const user of db.users) {
    ...
    const assigned = coachingStore.db.prepare(`SELECT o.prescription_json FROM workout_occurrences o JOIN client_relationships r ON r.id=o.relationship_id WHERE r.client_id=? AND r.status='active' AND o.scheduled_date=? AND o.state='scheduled' LIMIT 1`).get(user.id,now.date);
    const rid = assigned ? null : effectiveRoutineId(S, now.date);
    if (!assigned && !rid) continue;
    const routine = (S.routines || []).find(r => r.id === rid);
    const assignedPlan = assigned ? JSON.parse(assigned.prescription_json) : null;
    console.log('reminder firing', user.id, assigned ? 'assigned' : rid);
    user.lastReminder = now.date;
    saveDb();
    sendPush(user.id, {
      title: assignedPlan ? `${assignedPlan.dayName} today` : routine ? `${routine.emoji || '🏋️'} ${routine.name} today` : 'Workout planned today',
...
const routes = {
  ...
  'GET /api/me': async (req, res) => {
    ...
    json(res, 200, { user: { id: user.id, name: user.name, admin: isAdmin(user), capabilities: { trainer: isTrainer(user), client: true } } });
  },

  'POST /api/register/options': async (req, res) => {
    const body = await readBody(req);
    if (!allowRegistrationIp(requestIp(req))) return json(res, 429, { error: 'too many registration attempts — try again shortly' }, { 'Retry-After': '600' });
    const name = String(body.name || '').trim().slice(0, 40);
    if (!name) return json(res, 400, { error: 'name required' });
    const code = String(body.code || '').trim().toUpperCase();
    const coachingTokenHash = body.coachingToken ? coachingInvitations.hashToken(body.coachingToken) : null;
    if (coachingTokenHash && !allowRegistrationToken(coachingTokenHash)) return json(res, 429, { error: 'too many attempts for this invitation — try again shortly' }, { 'Retry-After': '600' });
    const adminAdmission = db.invites.some(i => i.code === code && !i.usedBy && !i.revoked);
    const coachingAdmission = coachingTokenHash && coachingInvitations.canAdmitHash(coachingTokenHash)
      && !db.coachingAdmissions.some(item => item.tokenHash === coachingTokenHash);
    if (INVITE_ONLY && !adminAdmission && !coachingAdmission)
    ...
    const cid = putChallenge({ challenge: options.challenge, name, uid, code, coachingTokenHash: coachingAdmission ? coachingTokenHash : null });
  ...
  'POST /api/register/verify': async (req, res) => {
    ...
    let invite = null;
    if (INVITE_ONLY && !c.coachingTokenHash) {
      invite = db.invites.find(i => i.code === c.code && !i.usedBy && !i.revoked);
      if (!invite) return json(res, 403, { error: 'invite code is no longer valid — ask for a new one' });
    }
    if (c.coachingTokenHash && (!coachingInvitations.canAdmitHash(c.coachingTokenHash)
      || db.coachingAdmissions.some(item => item.tokenHash === c.coachingTokenHash))) {
      return json(res, 403, { error: 'coaching invitation is no longer valid — sign in or ask for a new one' });
    }
    ...
    if (c.coachingTokenHash) db.coachingAdmissions.push({ tokenHash: c.coachingTokenHash, createdUserId: user.id, createdAt: user.created });
    saveDb();
    json(res, 200, { user: { id: user.id, name: user.name, admin: isAdmin(user), capabilities: { trainer: isTrainer(user), client: true } } }, { 'Set-Cookie': sessionCookie(user) });
  },
  ...
  'GET /api/data': async (req, res) => {
    ...
    try {
      ...
      json(res, 200, { state: projectAssignedResults(user.id, state) });
  ...
  'PUT /api/data': async (req, res) => {
    ...
    delete body.state.capabilities; delete body.state.relationships; delete body.state.trainer; delete body.state.coaching;
    body.state = projectAssignedResults(user.id, body.state);
    atomicWrite(stateFile(user.id), JSON.stringify(body.state));
    coachingStore.db.prepare(`INSERT INTO sync_receipts(client_id,personal_state_received_at) VALUES (?,?) ON CONFLICT(client_id) DO UPDATE SET personal_state_received_at=excluded.personal_state_received_at`).run(user.id,new Date().toISOString());
  ...
  'POST /api/push/test': async (req, res) => {
    const user = readSession(req);
    if (!user) return json(res, 401, { error: 'not signed in' });
    await sendPush(user.id, { title: 'transforma', body: 'Test notification ✅ — this is what alerts look like.', tag: 'test' });
    json(res, 200, { ok: true });
  },
  ...
  ...coachingRoutes({ handlers: coachingHandlers, json, readBody, readSession: readCoachingSession, config: coachingConfig, store: coachingStore })
};
