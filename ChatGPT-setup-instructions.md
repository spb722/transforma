...
This guide is for the person who owns an transforma server and wants to run its AI Coach using a
...
- a current transforma deployment built from this source revision;
- an transforma administrator profile;
...
1. Sign in to transforma and open **Settings → Admin dashboard → AI Coach**.
2. Turn on **AI Coach**.
...
7. The sheet shows a link and a one-time verification code. On your iPad or another trusted
   ...
   the code.
8. Return to transforma. The sheet closes after Codex saves its local login, and the credential card
   shows **connected**.
9. Select **Test the Coach**. A green result confirms an actual Codex response rather than only
   a successful sign-in.

The browser authorization happens between you and OpenAI. transforma only starts the local Codex
...
- restrict access to the transforma admin dashboard to people you trust with the ChatGPT account.
