...
- [X] T096a Shared coaching CSS foundation extended to the blockprint/eyebrow language by analogy (no literal 2a spec covers these screens): `frontend/src/components/coaching/coaching.css` — `.coaching-stat-grid`/`.coaching-section-grid`/`.coaching-client-list`/`.coaching-invite-row` converted from bordered cards/dividers to abutting 2px-gap blocks with no border; stat-grid labels became uppercase tracked eyebrows and numbers gained tabular-nums; `.coaching-panel`/`.coaching-hero`/`.coaching-banner` lost their borders (kept their normal standalone-section spacing — they are not a related group, so they don't get the 2px treatment). Because this is a shared foundation, it cascades to every screen that consumes it (`TrainerHome`, `ClientHome`, `ClientWorkspace`, `WorkoutPlans`, `DietPlans`, `Invitations`, `Fees`, `Progress`) without touching their individual JSX — confirmed via live screenshots of all of them plus the full e2e regression suite. Along the way, caught and fixed a real regression from T085/T086/T088 (this session's own earlier work): several genuine `<h2>`/`<h4>` section headings had been turned into plain `<div>`s while applying `.eyebrow` styling, which broke both screen-reader heading navigation and `e2e/03-workouts.mjs`'s `.today-row` selector (renamed to `.trainer-block`) — both fixed.
- [ ] T096b **Individual coaching-screen JSX** — not done, by explicit choice (shared-foundation level judged sufficient for now). Fees, DietPlans, Invitations' own row layout, TrainerSettings, CheckInResponse, and WeeklyCheckIn (which uses its own bespoke bordered grid, not `.coaching-stat-grid`) still have their original JSX structure — they inherit T096a's visual foundation but haven't had the same close, per-screen attention (converting specific muted-text patterns to eyebrows, blocklist row conversions, etc.) that T085–T091 gave the personal-tracker screens. Same applies to `Login`, `Coach`/`CoachProposal`/`CoachIntake`, and `Admin`/`AdminCoach`, untouched by either T096a or T096b.
