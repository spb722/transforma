...
export async function passkeyRegister(name, code, coachingToken) {
  const { cid, options } = await api('/api/register/options', { method: 'POST', body: JSON.stringify({ name, code: code || '', coachingToken: coachingToken || '' }) })
