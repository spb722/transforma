...
//
// Two jobs:
//  1. A small, self-contained file a friend can import into THEIR transforma — just the
...
export function buildPlanBundle(S, name) {
  ...
  return { transforma_plan: PLAN_FMT, exported: todayISO(), name: name || '', week, routines, customEx }
}
...
export function parsePlan(raw) {
  ...
  if (!data || !data.transforma_plan || !Array.isArray(data.routines)) {
    throw new Error(t('this isn’t a transforma plan file'))
  }
...
<body><div class="doc">
  <header>
    <div class="kicker">transforma</div>
  ...
  ${body}
  <footer>${esc(t('Made with transforma'))}</footer>
</div></body></html>`
}
