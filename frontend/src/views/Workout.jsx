...
import { startFlow, beginAssignedWorkout, exercisePicker, exConfigSheet, exerciseDetailSheet, topWeightSheet, finishWorkout, workoutCompleteSheet, confirmSheet } from '../sheets.jsx'
import Icon from '../components/Icon.jsx'
import { Button, Check, NumberField, EmptyState } from '../components/ui.jsx'
...
function StartChooser() {
  ...
  const assignedSchedule = ownAssignmentSchedule(ownWorkspace)
  const assignedToday = assignedSchedule.today
  const todayStatus = assignedSchedule.todayStatus
  ...
  if (coachPlanActive) return <div className="narrow">
    <div className="hdr"><div><h1>{t('Start workout')}</h1><div className="sub">{t('Follow your coach-assigned schedule')}</div></div></div>
    {assignedToday ? <div className="card" style={{ borderColor: 'var(--trainer-text)' }}>
      <h2 className="accent">{assignedToday.state === 'started' ? t('Coach workout in progress') : t("Today's coach assignment")}</h2>
      <div className="row between" style={{ marginBottom: 12 }}><div><div className="big">{assignedToday.prescription.dayName}</div><div className="muted small">{assignedToday.prescription.planName} · {assignedToday.prescription.exercises.length} {t('exercises')} · {assignedToday.scheduledDate} · {ownWorkspace.relationship.timeZone}</div></div><span className="lrow-i"><Icon name="dumbbell" /></span></div>
      <Button variant="primary" icon="play" onClick={async () => { setAssignedError(''); try { await beginAssignedWorkout(assignedToday) } catch (error) { setAssignedError(error.message || 'Reconnect and try again') } }}>{assignedToday.state === 'started' ? t('Resume coach workout') : t('Start coach assignment')}</Button>
      {assignedError && <div className="small" role="alert" style={{ color: 'var(--red)', marginTop: 8 }}>{assignedError}</div>}
    </div> : <div className="card">
      <EmptyState icon="moon" description={todayStatus?.state === 'canceled' ? t('Your trainer has given you a day off today.') : t('No coach workout is scheduled for today.')} />
      <Button onClick={() => nav('/plan')}>{t('View coach schedule')}</Button>
    </div>}
  </div>
  return <div className="narrow">
    ...
    {assignedToday && <div className="card" style={{ borderColor: 'var(--trainer-text)' }}>
      <h2 className="accent">{t("Today's coach assignment")}</h2>
      <div className="row between" style={{ marginBottom: 12 }}><div><div className="big">{assignedToday.prescription.dayName}</div><div className="muted small">{assignedToday.prescription.planName} · {assignedToday.prescription.exercises.length} {t('exercises')} · {assignedToday.scheduledDate} · {ownWorkspace.relationship.timeZone}</div></div><span className="lrow-i"><Icon name="dumbbell" /></span></div>
      <Button variant="primary" icon="play" onClick={async () => { setAssignedError(''); try { await beginAssignedWorkout(assignedToday) } catch (error) { setAssignedError(error.message || 'Reconnect and try again') } }}>{t('Start coach assignment')}</Button>
      {assignedError && <div className="small" role="alert" style={{ color: 'var(--red)', marginTop: 8 }}>{assignedError}</div>}
    </div>}
    {todayR && <div className="card" style={{ borderColor: assignedToday ? undefined : 'var(--acc)' }}>
...
function ExerciseBlock({ entryIdx, compact, locked, onToggle, onField, onAddSet, onRemoveSet, onStartTimed }) {
  const S = useStore(s => s.S)
  const working = useUI(s => s.work)
  const entry = S.active.entries[entryIdx]
  const ex = exOr(entry.id)
  const displayUnit = S.active.unit || S.unit
  ...
  const col1 = cardio ? { f: 'min', step: 1, dec: false, hd: t('Duration (min)') }
    : timed ? { f: 'sec', step: 5, dec: false, hd: t('Seconds') }
      : { f: 'w', step: 2.5, dec: true, hd: t('Weight ({0})', displayUnit) }
  const col2 = cardio ? { f: 'speed', step: 0.5, dec: true, hd: t('Speed (km/h)') }
    : timed ? { f: 'w', step: 2.5, dec: true, hd: t('Weight ({0})', displayUnit) }
  ...
  return <>
    ...
    <div className="row" style={{ gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
      ...
      {best > 0 && <span className="tag nocap">{t('Best:')} {fmtNum(best)} {displayUnit}</span>}
    </div>
    ...
    {entry.target?.notes && <div className="card small" style={{ margin: '8px 0' }}><strong>{t('Coach note')}</strong><div className="muted">{entry.target.notes}</div></div>}
...
function ActiveWorkout() {
  ...
  const A = S.active
  const assignedSession = Boolean(A.assignedSessionId)
  ...
  const toggle = (idx, i) => {
    ...
    mutEntry(idx, e => {
      ...
      if (e.sets[i].done) {
        ...
        if (e.sets.every(x => x.done)) { exJustDone = true; if (!A.assignedSessionId && m === 'reps' && !e.asked) { e.asked = true; askTop = true } }
      }
    })
  ...
  // Per the approved design export's own stated rule: the in-session screen stays on
  // the dark/ink ground in both appearance modes, for legibility at arm's length in
  // gym lighting (spec.md Edge Cases). Restores whatever theme was active on unmount —
  // this only overrides the DOM attribute App.jsx's Shell already manages, never the
  // client's stored preference.
  useEffect(() => {
  ...
  return <div className="narrow">
      ...
      {isSuperset ? (
        <div className="ss-card">
          ...
          {unit.map((idx, k) => <div key={idx} className="ss-ex">
            ...
            <ExerciseBlock entryIdx={idx} compact locked={assignedSession}
        ...
        <ExerciseBlock entryIdx={cur} locked={assignedSession} onToggle={i => toggle(cur, i)} onField={(i, f, v) => setField(cur, i, f, v)} onAddSet={() => addSet(cur)} onRemoveSet={() => removeSet(cur)} onStartTimed={i => startTimed(cur, i)} />
      )}
    </> : <EmptyState icon="shuffle" description={t('Freestyle workout — add your first exercise.')} />}
    ...
    <div style={{ height: 10 }} />
    {!assignedSession && <Button onClick={() => exercisePicker(ex => exConfigSheet(ex, null, cfg => update(s => {
