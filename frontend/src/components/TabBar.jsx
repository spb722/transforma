...
export default function TabBar({ onStart }) {
  ...
  if (cur === 'studio' || cur === 'coaching' || cur === 'join') return null
  const on = k => cur === k || (cur === 'history' && k === 'stats') || (cur === 'settings' && k === 'home')

  const startWorkout = () => {
    if (!S.active) {
      const r = effectiveRoutine(S, todayISO())
      if (!coachPlanActive && r && r.ex.length) { onStart(r.id); return }
    }
    nav('/workout')
  }
  ...
  return (
    <nav id="tabbar">
      ...
      <button className={'start' + (on('workout') ? ' on' : '') + (S.active ? ' rec' : '')} onClick={startWorkout}>
        <Icon name={S.active ? 'play' : 'dumbbell'} /><span>{S.active ? t('Resume') : t('Start')}</span>
      </button>
