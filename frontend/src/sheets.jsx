...
import { EXIDX, BODYPARTS, isCardio, allExercises } from './lib/exercises.js'
...
import { coachingApi, mutationId } from './lib/coaching/api.js'
import { assignedOutbox } from './lib/coaching/outbox.js'
import { assignedWorkoutFromSnapshot, resultPayload } from './lib/coaching/logger.js'
...
function ExercisePicker({ onPick, close }) {
  return <>
    ...
    <ExerciseCatalogPicker onPick={onPick} onManual={() => customExSheet(null, ex => onPick(ex))} />
  </>
}
...
function PlanTools({ close }) {
  ...
  const exportFile = async () => {
    ...
    const name = 'transforma-plan-' + todayISO() + '.json'
  ...
  return <>
    ...
    <div className="dim small" style={{ margin: '7px 2px 0', lineHeight: 1.4 }}>{t('A small file a friend imports into their own transforma — routines only, none of your workouts or weigh-ins.')}</div>
    {!MOBILE && <>
      <div style={{ height: 12 }} />
...
function WorkoutDetail({ w, close }) {
  const st = useStore(s => s.S)
  const displayUnit = w.assignedPrescription?.unit || st.unit
  return <>
    <h3>{w.name}</h3>
    <div className="muted small" style={{ marginBottom: 12 }}>{[fmtDate(w.d, true), ...durPart(w.end - w.start), fmtVol(w.vol, displayUnit), ...(w.bw ? [fmtNum(w.bw) + ' ' + displayUnit] : []), ...(w.assignedSessionId ? [w.assignedSync === 'synced' ? t('Coach assigned · synced') : t('Coach assigned · waiting to sync')] : [])].join(' · ')}</div>
    {w.entries.map((e, i) => {
      const ex = EXIDX[e.id]
    ...
    {w.assignedSessionId ? <div className="card small muted">{t('Assigned results are read-only after synchronization so the trainer and client keep the same record.')}</div> : <Button variant="danger" onClick={() => confirmSheet({ title: t('Delete workout?'), message: t('This removes it from your history for good.'), confirmText: t('Delete'), danger: true, onConfirm: () => { update(s => { s.workouts = s.workouts.filter(x => x.id !== w.id) }); close(); toast(t('Workout deleted')) } })}>{t('Delete workout')}</Button>}
  </>
}
...
function Calendar({ start, close }) {
  ...
  for (let d = 1; d <= daysIn; d++) {
    ...
    const coachOccurrence = coachPlanActive && ownWorkspace.workout.occurrences?.find(item => item.scheduledDate === iso)
    const dotCls = ws ? 'done' : ['scheduled', 'started'].includes(coachOccurrence?.state) ? 'plan' : coachOccurrence?.state === 'canceled' ? 'ovr' : !coachPlanActive && ovr && effId ? 'ovr' : !coachPlanActive && effId ? 'plan' : ''
    cells.push(<button key={d} className={'cal-d' + (ws ? ' has' : '') + (iso === todayISO() ? ' today' : '')} onClick={() => {
      if (!ws) { if (!coachPlanActive) { close(); dayOverrideSheet(iso) }; return }
  ...
  return <>
    ...
    <div className="small dim" style={{ textAlign: 'center', marginTop: 10 }}>{coachPlanActive ? t('Your coach manages this schedule. Tap a trained day for details.') : t('Tap a trained day for details · tap any other day to plan a session')}</div>
  </>
}
...
export function WorkoutRow({ w, onClick }) {
  ...
  return <div className="item" onClick={onClick}>
    ...
    <div className="grow"><div className="tt">{w.name}</div>
      <div className="ss">{[w.assignedSessionId ? t('Coach assigned') : null, fmtDate(w.d, true), ...durPart(w.end - w.start), t('{0} sets', setsDone(w)), fmtVol(w.vol, displayUnit), w.assignedSessionId ? (w.assignedSync === 'synced' ? t('synced') : t('waiting to sync')) : null].filter(Boolean).join(' · ')}</div></div>
...
export async function beginAssignedWorkout(occurrence) {
  const user = useStore.getState().user
  if (!user) throw new Error('Sign in to start an assigned workout')
  const workspace = useCoachingStore.getState().ownWorkspace
  let started = occurrence.state === 'started' && workspace?.startedSession?.occurrenceId === occurrence.id
    ? workspace.startedSession
    : null
  if (!started && occurrence.state === 'started' && workspace?.relationship?.id) {
    const sessions = await coachingApi(`/sessions?relationshipId=${encodeURIComponent(workspace.relationship.id)}`)
    started = sessions.items?.find(item => item.state === 'started' && item.occurrenceId === occurrence.id) || null
  }
  if (!started && occurrence.state === 'started') throw new Error('This workout is already in progress. Refresh and try Resume again.')
  if (!started) {
    const sessionId = mutationId()
    started = await coachingApi('/sessions/start', { method: 'POST', body: { sessionId, occurrenceId: occurrence.id, expectedOccurrenceVersion: occurrence.version } })
  }
  const active = assignedWorkoutFromSnapshot(started.sessionId, started.prescriptionSnapshot)
  update(s => {
    for (const exercise of started.prescriptionSnapshot.exercises || []) {
      const exerciseId = exercise.exerciseId || exercise.id
      if (!EXIDX[exerciseId] && !(s.customEx || []).some(item => item.id === exerciseId)) s.customEx.push({ id: exerciseId, n: exercise.name, bp: exercise.custom?.muscle || '', tg: exercise.custom?.muscle || '', eq: exercise.custom?.equipment || '', sm: [], st: [], custom: true })
    }
    s.active = { ...active, d: started.scheduledDate, start: Date.parse(started.startedAt), routineId: null, name: started.prescriptionSnapshot.dayName || 'Assigned workout', bw: null, cur: 0, relationshipId: started.relationshipId, occurrenceId: started.occurrenceId, assignedSync: 'started' }
  })
  useUI.getState().stopRest()
  nav('/workout')
}
function TopWeight({ entryIdx, close }) {
  const st = useStore(s => s.S)
  const A = st.active
  const displayUnit = A?.unit || st.unit
  ...
  const commit = advance => {
    ...
    } else toast(t('Tracked — next time starts at {0}', fmtNum(S().exWeights[entry.id].w) + ' ' + displayUnit))
  }
  return <>
    ...
    <WeightInput value={v} setValue={setV} unit={displayUnit} />
    <div style={{ height: 10 }} />
    {prevBest > 0 ? <div className="small dim" style={{ textAlign: 'center', marginBottom: 12 }}>{t('Previous best:')} {fmtNum(prevBest)} {displayUnit}{maxSet > prevBest && <span style={{ color: 'var(--yellow)' }}> — {t('new record!')}</span>}</div> : <div style={{ height: 4 }} />}
    {unitDone ? <>
...
function FinishSummary({ w, prs, e1prs = [], close }) {
  ...
  return <div style={{ textAlign: 'center', padding: '8px 0' }}>
    ...
    {(prs.length > 0 || e1prs.length > 0) && <div style={{ textAlign: 'left', marginBottom: 12 }}>
      ...
      {e1prs.map(p => <div key={p.id} className="small accent capitalize row" style={{ gap: 5 }}><Icon name="chartLine" style={{ fontSize: 13 }} />{t('Best estimated 1RM:')} {(EXIDX[p.id] || {}).n || p.id} · {fmtNum(p.est)} {displayUnit}</div>)}
    </div>}
...
function doFinishWorkout() {
  ...
  const prs = []
  const e1prs = []
  if (!A.assignedSessionId) A.entries.forEach(e => {
  ...
  if (A.assignedSessionId) {
    try {
      assignedOutbox.enqueue(useStore.getState().user?.id, resultPayload(A))
      w.assignedSessionId = A.assignedSessionId
      w.assignedPrescription = A.assignedPrescription
      w.relationshipId = A.relationshipId
      w.occurrenceId = A.occurrenceId
      w.assignedSync = 'waiting'
    } catch {
      toast(t('Could not save this result on this device. Your workout is still open.'))
      return
    }
  }
  update(s => {
    if (!A.assignedSessionId) w.entries.forEach(e => {
  ...
  useUI.getState().stopRest()
  if (A.assignedSessionId) assignedOutbox.flush(useStore.getState().user.id, item => coachingApi('/sessions/result', { method: 'PUT', body: { ...item, queuedAt: undefined } }))
    .then(() => update(s => { const saved=s.workouts.find(item=>item.id===A.id); if(saved)saved.assignedSync='synced' }))
    .catch(() => {})
