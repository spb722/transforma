# Self-hosting transforma

transforma is two small containers (a web server and an API) plus a folder of your data.
...
```bash
git clone https://github.com/spb722/transforma
cd transforma
cp .env.example .env
...
transforma signs you in with **passkeys** (WebAuthn). Browsers enforce two rules:
...
passkey prompt won't appear. To use transforma from your phone you need a real HTTPS hostname.
...
Put transforma behind something that terminates TLS for a hostname you control, then point it at
...
transforma only needs the browser to reach it over `https://gym.example.com`.
...
Everything is in `./data`, including identities, personal workout files, coaching SQLite data,
session secrets, push keys, and optional provider credentials. Stop the API before copying the
directory so the JSON and SQLite files represent one point in time:

```bash
docker compose stop api
tar czf transforma-backup-$(date +%F).tar.gz data/
docker compose start api
```

Take a backup before every upgrade that may apply a migration, and take one daily while coaching
clients are active. A database-only copy is incomplete. To restore, stop the API, move the current
`data/` aside, unpack the full archive, and start the API. Confirm sign-in, a client workspace,
assigned history, and fee events before deleting the previous directory. If a migration fails,
leave the failed data untouched and restore or fix forward with a tested migration. Do not delete
the SQLite file to force startup. Individual JSON exports remain useful for a person's records,
but they cannot restore relationships, assigned-plan authority, or fee audit history.

## 6. Trainer Studio

Trainer capability is provisioned separately from administrator access. First register each
trainer normally, read their account id from `/api/me` or `data/db.json`, then set exact ids:

```bash
TRAINER_UIDS=trainer-account-id,another-trainer-id
```

Restart with `docker compose up -d`. The trainer completes their display name, business timezone,
and optional WhatsApp number in **Settings → Trainer Studio**, then creates a seven-day client
invitation. Clients retain their personal tools and history after joining. Either party may end
the relationship, which immediately removes trainer access while keeping the client's archive.

Fees use operator-created HTTPS checkout links. Configure exact lowercase hostnames and supported
currency exponents before a trainer can add one:

```bash
COACHING_PAYMENT_HOSTS=pay.example.com,checkout.example.org
COACHING_CURRENCIES={"INR":2,"USD":2,"JPY":0}
```

Opening or returning from a checkout never marks a fee paid. The trainer must verify the receipt
outside transforma and explicitly confirm it. Payment status never gates workouts, diets, check-ins,
or history.

The coaching store uses SQLite on the local `./data` volume and is designed for one API process.
Do not run multiple API replicas against this volume or put it on an unreliable network file
system. For larger deployments, replace the persistence layer before adding replicas.

This repository is AGPL-3.0. Operators who distribute modified versions or provide the covered
service over a network must review and satisfy the repository's license and source obligations.
Exercise images and GIFs come from the separately sourced media dataset; verify its license and
attribution for the version you deploy. The prototype in `design/coaching-prototype/` is a design
reference and is not the application served by Compose.

## 7. Notifications

transforma can push two kinds of alert to your phone/desktop, even when the app isn't open:
...
2. Pick one built-in provider:
   - **Claude Code**: on a trusted computer where you use Claude Code, run `claude setup-token`.
     ...
     in transforma. The token is encrypted at rest and passed only to the isolated Agent SDK job.
   - **OpenAI Codex CLI**: choose **Sign in with ChatGPT**, then use the link and one-time code
     on a trusted browser or iPad. This is Codex's device-code login; transforma never receives a
