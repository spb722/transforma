...
transforma ships in two flavors from the same codebase:
...
State is mirrored from `localStorage` into `transforma-state.json` in the app's private data
...
- **iOS:** a Mac with Xcode 15+ and CocoaPods (`brew install cocoapods`). A free Apple ID
  ...
  for App Store distribution, which transforma doesn't do.
...
```sh
cd frontend
npx @capacitor/assets generate --iconBackgroundColor '#201e1d' --splashBackgroundColor '#201e1d'
```
...
transforma's mobile app is not on the Play Store or App Store, and that's a choice: no store
accounts, no store rules, no yearly fees between you and an open-source app.

### Android — sideload the APK

The official signed APK is at **[github.com/spb722/transforma/releases](https://github.com/spb722/transforma/releases)**.
...
keytool -genkeypair -keystore my.keystore -alias transforma -keyalg RSA -validity 10950

# align + sign (zipalign/apksigner ship with the Android SDK build-tools)
zipalign -f -p 4 app-release-unsigned.apk aligned.apk
apksigner sign --ks my.keystore --ks-key-alias transforma --out transforma.apk aligned.apk
```
...
- **License:** transforma is AGPL-3.0, which by itself sits badly with app-store terms of
