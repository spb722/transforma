...
transforma is a self-hosted app: you run the server, you hold the data. This file says which
...
<https://github.com/spb722/transforma/security/advisories/new>
...
- **The published images** `ghcr.io/spb722/transforma-api` and `-web`.
...
Read this before hosting transforma for anyone other than yourself.
...
- **Nothing in `./data` is encrypted.** It holds `db.json` (users, passkey public keys, push
  ...
  `secret` can mint a valid session cookie for any account. **If you host transforma for other
